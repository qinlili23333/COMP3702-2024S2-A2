...
When prompted for an action, press W to move the bee
 forward, S to move the bee
  in reverse, A to turn the bee

left (counterclockwise) and D to turn the bee
 right (clockwise). Use Q to exit the simulation, and R to reset the
...
def main(arglist):
    ...
    while True:
        env.render(state)
        print("Press 'W' to move the bee forward, 'S' to move the bee in reverse, 'A' to turn the bee left "
              "(counterclockwise) and 'D' to turn the bee right (clockwise). Use '[' to exit the simulation, and ']' "
